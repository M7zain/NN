...
/** @param {number} x @param {number} [digits=6] */
 ...
 */
export function formatDerivativeKatex(act, zVal, aVal, fpiVal, fmtDigits = 6) {
 ...
 */
export function explainForwardPass(a, z, W, B, act, fmtDigits = 6) {
 ...
 */
export function explainBackwardPass(fwd, target, W, act, bp, fmtDigits = 6) {
  const f = (x) => fmt(x, fmtDigits);
  const { a, z } = fwd;
  const { E, deltas } = bp;
  const L = W.length;
      ...
      steps: [
        {
          ...
          tex: `E_{${i + 1}} = d_{${i + 1}} - y_{${i + 1}}`,
          plain: `E_${i + 1} = ${f(di)} - ${f(yi)} = ${f(Ei)}`,
        },
        {
        ...
        },
        {
          ...
          tex: `\\delta_{${i + 1}}^{(${L})} = E_{${i + 1}} \\cdot f'(\\mathrm{Net}_{${i + 1}}^{(${L})})`,
          plain: `δ_${i + 1}^(${L}) = ${f(Ei)} × ${f(fpi)} = ${f(del)}`,
          chainDetails: [
            {
              label: "Zincir (E → y → z)",
              tex: String.raw`\delta_i^{(L)}=\frac{\partial E}{\partial z_i^{(L)}}=\underbrace{\frac{\partial E}{\partial y_i}}_{\approx E_i}\cdot\frac{dy_i}{dz_i^{(L)}}`,
              plain: `Ödev notasyonu: E_i = d_i - y_i;  dy_i/dz_i = f'(z_i).`,
            },
            {
              label: "Sayısal çarpım",
              tex: String.raw`\delta_i^{(L)} = E_i\cdot f'\big(z_i^{(L)}\big)`,
  ...
  for (let lv = L - 1; lv >= 1; lv--) {
    ...
    for (let j = 0; j < a[lv].length; j++) {
      ...
      const partialChainPlain = backTerms
        .map(
          (t) =>
            `(∂E/∂z_${t.kIndex}^{(${lv + 1})})·(∂z_${t.kIndex}^{(${lv + 1})}/∂a_${j + 1}^{(${lv})}) = δ_${t.kIndex}·W_${t.kIndex},${j + 1} = ${f(t.product)}`
        )
        .join("; ");
      const chainDetails = [
        {
          label: "δ tanımı (zincir kuralı)",
          tex: String.raw`\delta_j^{(\ell)}=\frac{\partial E}{\partial z_j^{(\ell)}}=\frac{\partial E}{\partial a_j^{(\ell)}}\cdot\frac{da_j^{(\ell)}}{dz_j^{(\ell)}}`,
          plain: "",
        },
        {
          label: "∂E/∂a — sonraki katmandan gelen sinyal",
          tex: String.raw`\frac{\partial E}{\partial a_j^{(\ell)}}=\sum_k\frac{\partial E}{\partial z_k^{(\ell+1)}}\cdot\frac{\partial z_k^{(\ell+1)}}{\partial a_j^{(\ell)}}=\sum_k\delta_k^{(\ell+1)}W_{k,j}^{(\ell+1)}`,
          plain: `Her k için: ∂E/∂z_k^(ℓ+1)=δ_k;  z_k^(ℓ+1)=Σ_j W_{k,j} a_j + b  ⇒  ∂z_k/∂a_j = W_{k,j}.  ${partialChainPlain}`,
        },
        {
          label: "da_j/dz_j",
          tex: String.raw`\frac{da_j^{(\ell)}}{dz_j^{(\ell)}}=f'\!\big(z_j^{(\ell)}\big)`,
          plain: `= ${f(fpl)}`,
        },
        {
          label: "Birleşik δ (çarpım)",
          tex: String.raw`\delta_j^{(\ell)}=\Big(\sum_k\delta_k^{(\ell+1)}W_{k,j}^{(\ell+1)}\Big)\cdot f'\!\big(z_j^{(\ell)}\big)`,
        ...
        steps: [
          {
            ...
            tex: `\\sum_{k} \\delta_{k}^{(${lv + 1})} W_{k,j}^{(${lv + 1})}`,
          ...
          },
          {
            ...
            tex: `\\delta_{${j + 1}}^{(${lv})} = \\Big(\\sum_k \\delta_k^{(${lv + 1})} W_{k,j}\\Big) f'\\!\\big(z_{${j + 1}}^{(${lv})}\\big)`,
            plain: `δ_${j + 1}^(${lv}) = ${f(sum)} × ${f(fpl)} = ${f(del)}`,
            chainDetails,
          },
        ],
